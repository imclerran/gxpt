...
- **Graphviz Diagrams**: Fenced graph code blocks are rendered inline as images using a small, portable Graphviz 2.38 (rasterized to PNG via GDI+). The layout engine is chosen by the fence language — ` ```dot ` for hierarchical graphs, or ` ```neato `/`fdp`/`twopi`/`circo`/etc. for the more compact force-directed, radial, and circular layouts. The rendered graph carries a slim header with a Copy button for the original DOT source, and **clicking the graph opens it in a pan/zoom viewer** (scroll to zoom, drag to pan, with a crisp high-DPI re-render) so dense diagrams stay readable. If Graphviz isn't bundled or the source can't be rendered, the block falls back to a normal highlighted code block. See `GxPT/lib/graphviz/README.md` for the (uncommitted) binaries to drop in.
...
- **Plugins**: Bundle interdependent skills and agents into a single, shareable `.gxpl` plugin you can install, upgrade, enable/disable, and uninstall by name. Installing fans the member skills and agents into your normal catalogs so they work unchanged. The **Manage Plugins** dialog (File > Plugins) and the `/plugin` command group cover the whole lifecycle, and double-clicking a `.gxpl` file installs it; a single skill or agent now exports as a one-item plugin too.

...
- **File Attachments**: Attach text files, **images**, and **PDFs** to your messages. Images are sent to vision-capable models (auto-normalized and downscaled for the wire); PDFs default to fast, token-light local text extraction and can be escalated to full-document sending on models that support it - scanned PDFs switch automatically. Attachments are stored with the conversation, so they stay viewable in-app and replay to the model even after the original file is gone. Capability and Zero Data Retention rules are enforced automatically, and a long conversation's images/PDFs are pruned oldest-first to stay within context while remaining viewable.

...
namespace GxPT
{
    ...
    internal sealed class McpChatOrchestrator
    {
        ...
        internal const string AgentSystemPrompt =
            ...
            + "When a tool call will tell you HOW to do a task - opening a skill for its instructions, or "
            + "reading a file whose contents will guide your approach - make that call by itself and wait "
            + "for its result BEFORE taking the actions it governs. Tool calls you issue together in one "
            + "turn run in parallel and cannot see each other's results, so opening a skill while "
            + "dispatching agents or running task tools in the SAME turn makes those actions run blind, "
            + "before the guidance arrives. Read first, then act on what you learned.\n\n"
